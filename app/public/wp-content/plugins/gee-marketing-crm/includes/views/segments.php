...
<div class="gee-crm-card">
    ...
    <div style="background:#f9f9f9; padding:15px; border:1px solid #ddd; margin-bottom:20px;">
        ...
        <form method="post" id="gee-segment-form">
            ...
            <div id="segment-preview" style="margin:20px 0; padding:15px; background:#e7f5ff; border:1px solid #2271b1; border-radius:4px; display:none;">
                <strong style="color:#2271b1;">Preview: </strong>
                <span id="segment-preview-count" style="font-size:18px; font-weight:bold; color:#2271b1;">0</span>
                <span style="color:#2271b1;"> contacts match these conditions</span>
                <button type="button" id="refresh-preview" class="button" style="margin-left:15px; padding:5px 15px;">Refresh Preview</button>
            </div>
                    ...
                    <?php if ( ! empty( $existing_conditions ) ) : ?>
                        <?php foreach ( $existing_conditions as $index => $condition ) : ?>
                            <div class="gee-condition-row" style="display:flex; gap:10px; margin-bottom:10px; align-items:center; padding:10px; background:#fff; border:1px solid #ddd; border-radius:4px;">
                                <select name="conditions[<?php echo esc_attr( $index ); ?>][field]" class="condition-field" style="flex:1; padding:5px;">
                                ...
                                <select name="conditions[<?php echo esc_attr( $index ); ?>][operator]" class="condition-operator" style="flex:1; padding:5px;" data-value="<?php echo esc_attr( $condition['operator'] ); ?>">
                                    <option value="">Select Operator...</option>
                                </select>
                                <div class="condition-value-wrapper" style="flex:2;">
                                    <?php if ( $condition['field'] === 'tag' ) : ?>
                                        <select name="conditions[<?php echo esc_attr( $index ); ?>][value]" class="condition-value" style="width:100%; padding:5px;">
                                            <option value="">Select Tag...</option>
                                            <?php foreach ( $tags as $tag ) : ?>
                                                <option value="<?php echo esc_attr( $tag->id ); ?>" <?php selected( $condition['value'], $tag->id ); ?>><?php echo esc_html( $tag->name ); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    <?php elseif ( $condition['field'] === 'total_purchase_value' || $condition['field'] === 'last_purchase_value' ) : ?>
                                        ...
                                        ?>
                                        <input type="<?php echo esc_attr( $input_type ); ?>" <?php echo $input_type === 'number' ? 'step="0.01"' : ''; ?> name="conditions[<?php echo esc_attr( $index ); ?>][value]" class="condition-value" value="<?php echo esc_attr( $condition['value'] ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>" style="width:100%; padding:5px;">
                                    <?php elseif ( in_array( $condition['field'], array( 'created_date', 'last_purchase_date', 'first_purchase_date' ) ) ) : ?>
                                        <?php 
                                        $operator = isset( $condition['operator'] ) ? $condition['operator'] : '';
                                        $is_date_picker = in_array( $operator, array( 'before', 'after' ) );
                                        $input_type = $is_date_picker ? 'date' : 'text';
                                        $placeholder = $is_date_picker ? 'Select Date' : 'Days or YYYY-MM-DD';
                                        $date_class = $is_date_picker ? 'date-picker' : '';
                                        ?>
                                        <input type="<?php echo esc_attr( $input_type ); ?>" name="conditions[<?php echo esc_attr( $index ); ?>][value]" class="condition-value <?php echo esc_attr( $date_class ); ?>" value="<?php echo esc_attr( $condition['value'] ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>" style="width:100%; padding:5px;">
                                    <?php else : ?>
                                        <input type="text" name="conditions[<?php echo esc_attr( $index ); ?>][value]" class="condition-value" value="<?php echo esc_attr( $condition['value'] ); ?>" placeholder="Value" style="width:100%; padding:5px;">
                                    <?php endif; ?>
                                </div>
        ...
        <tbody>
            <?php if ( $segments ) : ?>
                <?php foreach ( $segments as $seg ) : ?>
                    ...
                    $contact_ids = $segment_model->get_contact_ids_in_segment( $seg->id );
                    $contact_count = count( $contact_ids );
                    ?>
                    <tr>
                        <td><?php echo esc_html( $seg->id ); ?></td>
                        ...
                        <td>
                            <a href="?page=gee-woo-crm&view=segments&action=edit&id=<?php echo esc_attr( $seg->id ); ?>" class="gee-crm-btn" style="margin-right:5px;">Edit</a>
                            <a href="?page=gee-woo-crm&view=segments&action=delete&id=<?php echo esc_attr( $seg->id ); ?>" class="gee-crm-btn" style="color:red; border-color:red;" onclick="return confirm('Delete?');">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else : ?>
                <tr><td colspan="5">No segments found.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
...
jQuery(document).ready(function($) {
    ...
    $(document).on('change', '.condition-operator', function() {
        ...
        if (field === 'tag') {
            ...
            var placeholder = 'Days or YYYY-MM-DD';
            var inputType = 'text';
            // Use date picker for 'before' and 'after' operators
            ...
            if ($valueInput.is('select')) {
                $valueInput.replaceWith('<input type="' + inputType + '" name="' + inputName + '" class="condition-value date-picker" placeholder="' + placeholder + '" style="width:100%; padding:5px;">');
            } else {
                $valueInput.attr('type', inputType).attr('placeholder', placeholder);
                if (inputType === 'date') {
                    $valueInput.addClass('date-picker');
                } else {
                    $valueInput.removeClass('date-picker');
                }
            }
            
            // Initialize date picker if needed
            if (inputType === 'date' && typeof $.fn.datepicker !== 'undefined') {
                $valueWrapper.find('.date-picker').datepicker({
                    dateFormat: 'yy-mm-dd',
                    changeMonth: true,
                    changeYear: true
                });
    ...
    $('.condition-field').each(function() {
        if ($(this).val()) {
            ...
            setTimeout(function() {
                if (operatorValue) {
                    $row.find('.condition-operator').val(operatorValue).trigger('change');
                }
            }, 100);
        }
    });
    
    // Initialize date pickers for existing date fields
    ...
    function updateSegmentPreview() {
        ...
        if (!hasValidConditions) {
            $('#segment-preview').hide();
            return;
        }
        
        var logic = $('#segment-logic').val();
        
        $.ajax({
            url: geeWooCRM.ajaxurl,
            type: 'POST',
            data: {
                action: 'gee_crm_preview_segment',
            ...
            success: function(response) {
                if (response.success && response.data && typeof response.data.count !== 'undefined') {
                    $('#segment-preview-count').text(response.data.count);
    ...
    // Auto-update preview when conditions change
    $(document).on('change', '.condition-field, .condition-operator, .condition-value', function() {
        clearTimeout(window.previewTimeout);
        window.previewTimeout = setTimeout(updateSegmentPreview, 500);
    });
    
    $(document).on('change', '#segment-logic', function() {
        updateSegmentPreview();
    });
    
    // Initial preview if editing
    <?php if ( $editing_segment && ! empty( $existing_conditions ) ) : ?>
    setTimeout(updateSegmentPreview, 1000);
    <?php endif; ?>
});
</script>
