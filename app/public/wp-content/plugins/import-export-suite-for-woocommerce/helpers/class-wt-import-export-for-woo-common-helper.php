...
class Wt_Import_Export_For_Woo_Common_Helper {
	 ...
	 *
	 * @param string $str Time string to convert.
	 ...
	 * @return string
	 */
	public static function wt_utf8_encode( $value ) {
	...
	public static function wt_unserialize_safe( $data ) {
		...
		if ( ! is_serialized( $data ) ) {
			if ( is_string( $data ) ) {
				...
				if ( ! json_last_error() && is_array( $formated_data ) ) {
					return $formated_data;
				}
			}
			// If not a string or JSON decode failed, return data as-is.
			return $data;
		}
