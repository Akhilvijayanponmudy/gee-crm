...
class Wt_Import_Export_For_Woo_Order_Export {
	...
	public function wt_line_item_separate_row_csv_data( $order_export_data, $order_data_filter_args ) {
		$row      = array();
		$order_id = isset( $order_export_data['order_id'] ) ? $order_export_data['order_id'] : null;

		if ( ! $order_id && isset( $order_export_data['order_number'] ) ) {
			$order_id = $order_export_data['order_number'];
		}
	...
	public function generate_row_data( $order_id ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$product = ( version_compare( WC_VERSION, '4.4.0', '<' ) ) ? $order->get_product_from_item( $item ) : $item->get_product();
			...
			$line_item = array(
				...
				'product_id' => ( version_compare( WC_VERSION, '2.7.0', '<' ) ) ? $product->id : ( ( 'variable' === $prod_type || 'variation' === $prod_type || 'subscription_variation' === $prod_type ) ? $product->get_parent_id() : $product->get_id() ),
				'sku'        => $product->get_sku(),
				'quantity'   => $item['qty'],
		...
		if ( version_compare( WC_VERSION, '2.7', '<' ) ) {
			...
			// Get order date - prefer WooCommerce method, fallback to post_date.
			$order_date = '';
			if ( method_exists( $order, 'get_date_created' ) ) {
				$date_created = $order->get_date_created();
				if ( $date_created ) {
					// Use wp_date() for WordPress 5.3.0+ (with UTC for export consistency), fallback to gmdate() for older versions.
					if ( function_exists( 'wp_date' ) ) {
						$order_date = wp_date( 'Y-m-d H:i:s', $date_created->getTimestamp() );
					} else {
						// wp version is less than 5.3.0.
						$order_date = gmdate( 'Y-m-d H:i:s', $date_created->getTimestamp() ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
					}
				}
			}

			// Fallback to post_date if WooCommerce method didn't work.
			if ( empty( $order_date ) ) {
				$order_post = get_post( $order->get_id() );
				if ( $order_post && isset( $order_post->post_date ) && ! empty( $order_post->post_date ) ) {
					$post_timestamp = strtotime( $order_post->post_date );
					if ( false !== $post_timestamp ) {
						// Use wp_date() for WordPress 5.3.0+ (with UTC for export consistency), fallback to gmdate() for older versions.
						if ( function_exists( 'wp_date' ) ) {
							$order_date = wp_date( 'Y-m-d H:i:s', $post_timestamp );
						} else {
							// wp version is less than 5.3.0.
							$order_date = gmdate( 'Y-m-d H:i:s', $post_timestamp ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
						}
					}
				}
			}
			...
			$order_data = array(
				...
				'shipping_phone'       => ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '5.6', '<' ) ) ? '' : ( method_exists( $order, 'get_shipping_phone' ) ? $order->get_shipping_phone() : '' ),
	...
	public static function hf_get_orders_of_products( $products, $export_order_statuses, $export_limit, $export_offset, $end_date, $start_date, $exclude_already_exported, $retun_count = false ) {
		...
		if ( self::$table_name === $wpdb->prefix . 'wc_orders' ) {
			// Build the query with SQL fragments concatenated directly (they're safe - either empty or already prepared).
			$order_ids = $wpdb->get_col(
				...
				AND (po.date_created_gmt BETWEEN %s AND %s)' . $export_order_statuses_qry . $exclude_already_exported_qry . $limit_offset_qry, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					array_merge( $products, array( $start_date, $end_date ) )
				)
			);
		} else {
			// Build the query with SQL fragments concatenated directly (they're safe - either empty or already prepared).
			$order_ids = $wpdb->get_col(
				...
				AND om.meta_key IN (\'_product_id\',\'_variation_id\')
				AND om.meta_value IN (' . implode( ',', array_fill( 0, count( $products ), '%d' ) ) . ')
				AND (po.post_date BETWEEN %s AND %s)' . $export_order_statuses_qry . $exclude_already_exported_qry . $limit_offset_qry, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					array_merge( $products, array( $start_date, $end_date ) )
				)
			);
		}
	...
	public static function format_data( $data ) {
		...
		$data = Wt_Import_Export_For_Woo_Common_Helper::wt_utf8_encode( $data );
	...
	public static function get_max_line_items( $order_ids ) {
		...
		// If no order IDs, return 0 to avoid SQL error.
		if ( empty( $order_ids ) ) {
			return 0;
		}
