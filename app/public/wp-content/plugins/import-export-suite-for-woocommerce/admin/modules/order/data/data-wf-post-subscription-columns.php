...
if ( ! defined( 'ABSPATH' ) ) {
	...
	if ( class_exists( 'HF_Subscription' ) ) {
		$sub_post_type = 'hf_shop_subscription';
	} else {
		$sub_post_type = 'shop_subscription';
	}
	...
	foreach ( $meta_keys as $meta_key ) {
		if ( empty( $columns[ $meta_key ] ) ) {
			if ( '_' === $meta_key[0] && empty( $columns[ substr( $meta_key, 1 ) ] ) ) {
				$columns[ 'meta:' . $meta_key ] = 'meta:' . $meta_key;
				// adding an extra prefix for identifying meta while import process.
			} elseif ( '_' !== $meta_key[0] ) {
				$columns[ 'meta:' . $meta_key ] = 'meta:' . $meta_key;
			}
		}
