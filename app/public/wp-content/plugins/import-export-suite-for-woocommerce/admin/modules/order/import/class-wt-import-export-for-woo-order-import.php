...
class Wt_Import_Export_For_Woo_Order_Import {
	...
	public function wt_order_existance_check( $id ) {
		...
		if ( $id ) {
			...
			if ( 1 === $this->is_sync ) {
				...
				if ( $data_from_post_table && $data_from_order_table ) {
				...
				} elseif ( $data_from_post_table && $post_table_type === $this->post_type && in_array( $post_table_status, $statuses ) && ! $data_from_order_table ) {
				...
				} elseif ( $data_from_order_table && $order_table_type === $this->post_type && in_array( $order_table_status, $statuses ) && ! $data_from_post_table ) {
					$postdata = array( // if not specifiying id (id is empty) or if not found by given id.
						...
						'ping_status'   => 'closed',
						'post_author'   => 1,
						'post_title'    => sprintf( 'Order &ndash; %s', date_i18n( 'M d, Y @ h:i A', strtotime( $order_table_date_created_gmt ) ) ),
			...
			} elseif ( $this->table_name === $wpdb->prefix . 'wc_orders' ) {
				...
				} elseif ( ! $data_from_post_table && $data_from_order_table ) {
	...
	public function wt_parse_shipping_items_field( $value ) {
		...
		if ( isset( $shipping_items['taxes'] ) ) {
			$tax_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $shipping_items['taxes'] );
	...
	public function wt_parse_tax_items_field( $value ) {
		...
		foreach ( $tax_item as $tax ) {
			...
			foreach ( explode( '|', $tax ) as $piece ) {
				list( $name, $value ) = array_pad( explode( ':', $piece ), 2, '' );
				if ( isset( $name ) ) {
			...
			if ( $tax_item_data['rate_id'] && ( ! isset( $tax_rates[ $tax_item_data['rate_id'] ] ) || 0 === $tax_item_data['rate_id'] ) ) {
				$tax_item_data['rate_id'] = 0;
			} elseif ( isset( $tax_item_data['label'] ) && isset( $tax_rates[ $tax_item_data['rate_id'] ] ) && is_object( $tax_rates[ $tax_item_data['rate_id'] ] ) && ( 0 !== strcasecmp( $tax_rates[ $tax_item_data['rate_id'] ]->tax_rate_name, $tax_item_data['label'] ) || $tax_rates[ $tax_item_data['rate_id'] ]->tax_rate !== $tax_item_data['rate_percent'] ) ) {
	...
	public function wt_parse_id_field( $data, $parsed_data ) {
		...
		if ( 1 === $this->is_sync ) {
			...
			$postdata = array( // if not specifiying id (id is empty) or if not found by given id.
				...
				'ping_status'   => 'closed',
				'post_author'   => 1,
				'post_title'    => sprintf( 'Order &ndash; %s', date_i18n( 'M d, Y @ h:i A', strtotime( $date ) ) ),
				'post_password' => wc_generate_order_key(),
		...
		try {
			...
			if ( ! empty( $data['order_items'] ) ) {
				foreach ( $data['order_items'] as $item ) {
					...
					$product_title       = __( 'Unknown Product', 'import-export-suite-for-woocommerce' );
					if ( $item['product_id'] ) {
						$product = wc_get_product( $item['product_id'] );
						if ( $product ) {
							$product_title = ( '' !== $product->get_title() ) ? $product->get_title() : __( 'Unknown Product', 'import-export-suite-for-woocommerce' );
						}
			...
			if ( ! empty( $data['coupon_items'] ) ) {
				if ( Wt_Import_Export_For_Woo_Common_Helper::wt_iew_is_woocommerce_prior_to( '2.7' ) ) {
					...
					foreach ( $data['coupon_items'] as $coupon ) {
						...
						$id    = ( $query->have_posts() ? $query->posts[0]->ID : '' );
	...
	public function wt_create_refund( $input_currency, $args = array() ) {
		...
		try {
			...
			$total_amount            = ! is_numeric( $args['amount'] ) ? floatval( $args['amount'] ) : $args['amount'];
			...
			$refund->set_total( $total_amount * -1 );
		...
		if ( isset( $data['meta_data'] ) ) {
			...
			foreach ( $data['meta_data'] as $meta ) {
				...
				if ( is_serialized( $meta['value'] ) ) {
					...
					$meta['value'] = ( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta['value'] ) );
				}
