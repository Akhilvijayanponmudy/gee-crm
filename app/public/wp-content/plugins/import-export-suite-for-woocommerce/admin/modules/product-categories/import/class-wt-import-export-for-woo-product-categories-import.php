<?php
/**
 * Handles the product categories import.
 *
 * @package ImportExportSuite\Admin\Modules\ProductCategories\Import
 * @version 1.0.0
 */
...
/**
 * Wt_Import_Export_For_Woo_Categories_Import Class
 */
class Wt_Import_Export_For_Woo_Product_Categories_Import {
	...
	public function __construct( $parent_object ) {
		...
		/**
		 * Filter to alter the product categories import columns
		 *
		 * @param array $taxonomy_post_defaults Taxonomy post defaults.
		 * @return array
		 * @since 1.0.0
		 */
		$this->taxonomy_post_defaults = apply_filters(
			'wt_categories_csv_product_post_columns',
			array(
				'term_id'     => 'term_id',
				'name'        => 'name',
				'slug'        => 'slug',
			...
			if ( ! is_wp_error( $parsed_data ) ) {
				...
				if ( ! is_wp_error( $result ) ) {
					...
					$this->import_results[ $row ] = array(
						...
						'message'    => $msg,
						'status'     => true,
						'status_msg' => __( 'Success', 'wt-import-export-for-woo' ),
						'post_id'    => $result['id'],
						'post_link'  => Wt_Import_Export_For_Woo_Product_Categories::get_item_link_by_id( $result['id'] ),
					);
					...
					$this->import_results[ $row ] = array(
						...
						'status'     => false,
						'status_msg' => __( 'Failed/Skipped', 'wt-import-export-for-woo' ),
						'post_id'    => '',
						'post_link'  => array(
							'title'    => __( 'Untitled', 'wt-import-export-for-woo' ),
							'edit_url' => false,
						),
					);
	...
	public function parse_product_categories( $data, $is_update, $taxonomy_type ) {
		try {
			...
			/**
			 * Filter to alter the product categories import data
			 *
			 * @param array $data Data.
			 * @return array
			 * @since 1.0.0
	...
	public function process_product_categories( $post, $is_update, $taxonomy_type ) {
		...
		try {
			...
			/**
			 * Action to run after product categories import
			 *
			 ...
			 * @since 1.0.0
			 */
			do_action( 'wt_ier_product_categories_import_inserted_object', $term_data, $post, $is_update, $taxonomy_type );
			return $term_data;
		} catch ( Exception $e ) {
			return new WP_Error( 'woocommerce_product_importer_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}//end process_product_categories()
			...
			if ( '' !== $term_id && ! $is_update ) {
				...
				if ( ! empty( $data['slug'] ) || '' !== $name ) {
					...
					if ( ! empty( $parent_id ) && empty( $res ) ) {
						...
						if ( $cid ) {
							$this->wt_update_category_yoast_data( $data, $cid );
							$this->wt_update_category_meta( $data, $cid );
						}
						$status = array(
							'id'     => $cid,
							'name'   => $name,
				...
				if ( $tid ) {
					$this->wt_update_category_yoast_data( $data, $tid );
					$this->wt_update_category_meta( $data, $tid );
				}
				$status = array(
					'id'     => $tid,
					'name'   => $name,
	...
	/**
	 * Method used for update yoast data
	 *
	 * @param array $data    data.
	 * @param int   $term_id term id.
	 */
	public function wt_update_category_yoast_data( $data, $term_id ) {
		...
		if ( class_exists( 'WPSEO_Options' ) ) {
			...
			if ( ! empty( $data['meta:_yoast_data'] ) ) {
				...
				$yoast_data_option['product_cat'][ $term_id ] = $yoast_data;
				update_option( 'wpseo_taxonomy_meta', $yoast_data_option );
			}
			unset( $data['meta:_yoast_data'] );
		}
	}//end wt_update_category_yoast_data()

	/**
	 * Method used for update category meta
	 *
	 * @param array $data    data.
	 * @param int   $term_id term id.
	 */
	public function wt_update_category_meta( $data, $term_id ) {

		foreach ( $data as $tm_key => $tm_value ) {
			if ( 'meta:' === substr( $tm_key, 0, 5 ) ) {
				$cat_meta_key = substr( $tm_key, 5 );
				update_term_meta( $term_id, $cat_meta_key, $tm_value );
			}
		}
	}//end wt_update_category_meta()
