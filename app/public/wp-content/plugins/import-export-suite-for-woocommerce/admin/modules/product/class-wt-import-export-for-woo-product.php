...
class Wt_Import_Export_For_Woo_Product {
	...
	private function get_product_brands() {
		 ...
		 *
		 * @since 1.4.0
		 * @param array $additional_taxonomies Additional brand taxonomies.
		 */
		...
		foreach ( $brand_taxonomies as $taxonomy ) {
			...
			try {
				...
				foreach ( $brands as $brand ) {
					...
					if ( ! is_object( $brand ) || ! isset( $brand->slug ) || ! isset( $brand->name ) || ! isset( $brand->term_id ) ) {
						continue;
					}

					// Skip if slug or name is empty.
	...
	public static function get_all_product_attributes( $post_type = 'product' ) {
		...
		if ( ! empty( $results ) ) {
			foreach ( $results as $_product_attributes ) {
				$attributes = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $_product_attributes ) );
	...
	public function exporter_alter_filter_fields( $fields, $base, $filter_form_data ) {
		...
		$fields['product_brand'] = array(
			'label'           => __( 'Product brand', 'import-export-suite-for-woocommerce' ),
			'placeholder'     => __( 'Any brand', 'import-export-suite-for-woocommerce' ),
			'field_name'      => 'product_brand',
			'sele_vals'       => $this->get_product_brands(),
			'help_text'       => __( 'Select specific product brands to export only products from those brands.', 'import-export-suite-for-woocommerce' ),
			'type'            => 'multi_select',
			'css_class'       => 'wc-enhanced-select',
			'validation_rule' => array( 'type' => 'text_arr' ),
		);
