...
class Wt_Import_Export_For_Woo_Product_Import {
	...
	public function wt_parse_id_field( $data ) {
		...
		$postdata                = array( // if not specifiying id (id is empty) or if not found by given id or Product.
			'post_title'   => ( 'variation' === $this->item_data['type'] ? 'product variation' : ( isset( $this->item_data['name'] ) ? $this->item_data['name'] : '' ) ),
		...
		if ( isset( $data['meta_data'] ) ) {
			foreach ( $data['meta_data'] as $meta ) {
				...
				if ( $has_setter ) {
					...
					continue;
				} elseif ( 0 === strpos( $meta['key'], '_' ) ) {
					// Handle cases when $meta['key'] already has a preceeding '_'.
					// Use 'set' instead of 'set_'.
					$function = 'set' . $meta['key'];
