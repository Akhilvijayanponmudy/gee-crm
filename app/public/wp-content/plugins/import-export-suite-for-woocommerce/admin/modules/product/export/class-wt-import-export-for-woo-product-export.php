...
class Wt_Import_Export_For_Woo_Product_Export {
	...
	public static function get_product_images( $product ) {
		...
		// Images.
		$images  = isset( $meta_data['_product_image_gallery'][0] ) ? explode( ',', Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['_product_image_gallery'][0] ) ) ) : false;
		$results = array();
		if ( $images ) {
		...
		$woo_variation_gallery_images = isset( $meta_data['woo_variation_gallery_images'][0] ) ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['woo_variation_gallery_images'][0] ) : false;
	...
	protected function generate_row_data_wc_lower( $product_object ) {
		...
		foreach ( $meta_data as $meta => $value ) {
			...
			$meta_value = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $value[0] ) );
		...
		if ( isset( $meta_data['_product_attributes'][0] ) ) {
			...
			$attributes = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['_product_attributes'][0] ) );
			...
			if ( ! empty( $attributes ) && is_array( $attributes ) ) {
				foreach ( $attributes as $key => $attribute ) {
					...
					$_default_attributes = isset( $meta_data['_default_attributes'][0] ) ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['_default_attributes'][0] ) ) : '';
			...
			if ( ! $export_columns || in_array( $value, $export_columns ) || in_array( $column, $export_columns ) ) {
				...
				if ( 'file_paths' === $column || 'downloadable_files' === $column ) {
					...
					if ( ! function_exists( 'wc_get_filename_from_url' ) ) {
						$file_paths = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['_file_paths'][0] ) );
						...
						$file_paths = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_data['_downloadable_files'][0] ) );
	...
	private function apply_brand_filter( &$args, $brands ) {
		...
		// Group brands by taxonomy - now handles proper colon format.
		...
		// Set relation for multiple tax queries (like in pro version).
