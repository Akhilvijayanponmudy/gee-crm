...
class Wt_Import_Export_For_Woo_Product_Review_Import {
	...
	public function parse_product_review( $data, $merge ) {
		try {
			...
			if ( ! $merge ) {
				...
				if ( $found_review_id ) {
					$usr_msg = "Product review skipped. Review with $found_review_id is already exists";
				...
				} elseif ( 0 === $this->use_sku && ( ! isset( $item['comment_post_ID'] ) || '' === $item['comment_post_ID'] ) ) {
			...
			*/
			...
			if ( $product_review['comment_ID'] && ! $merge ) {
				$product_review['comment_ID'] = $this->create_review_with_given_id( $product_review );
			}
	...
	public function process_product_reviews( $post ) {
		try {
			...
			if ( $merge && ! empty( $processing_product_id ) ) {
				...
				$comment_parent_session = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( get_option( 'wf_prod_review_alter_id' ) );
				// Ensure $comment_parent_session is always an array to avoid PHP 8.1+ deprecation warning.
				...
				if ( ! isset( $comment_parent_session['wt_review_basic'] ) || ! is_array( $comment_parent_session['wt_review_basic'] ) ) {
						$comment_parent_session['wt_review_basic'] = array();
				}
	...
	/**
	 * Create review with given id
	 *
	 * @global object $wpdb
	 * @param  array $product_review Product review.
	 */
	public function create_review_with_given_id( $product_review ) {
		global $wpdb;
		...
		return $return;
	}//end create_review_with_given_id()
}//end class
