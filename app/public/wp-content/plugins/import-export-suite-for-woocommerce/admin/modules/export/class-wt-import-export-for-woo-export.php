...
class Wt_Import_Export_For_Woo_Export {
	...
	public function process_upload( $step, $export_id, $to_export ) {
		...
		$form_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $export_data['data'] );
	...
	public function process_image_export( $form_data, $step, $to_process, $file_name = '', $export_id = 0, $offset = 0 ) {
		...
		if ( $export_id > 0 ) {
			...
			$form_data = ( isset( $export_data['data'] ) ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $export_data['data'] ) : array() );
		}
			...
			if ( Wt_Iew_Sh::check_write_access( WT_IEW_PLUGIN_ID ) ) {
				...
				if ( '' !== $file_name ) {
					...
					if ( isset( $this->allowed_export_file_type[ $file_ext ] ) || 'zip' === $file_ext ) {
						...
						if ( file_exists( $file_path ) && is_file( $file_path ) ) { // nosemgrep
							...
							header( 'Content-Length: ' . filesize( $file_path ) ); // nosemgrep.
							readfile( $file_path ); // nosemgrep.
							exit();
