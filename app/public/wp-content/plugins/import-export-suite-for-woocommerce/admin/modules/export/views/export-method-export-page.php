			...
			<td colspan="2" style="width:75%;">
				<div class="wt_iew_radio_block">
					...
					foreach ( $this->export_obj->export_methods as $key => $value ) {
						?>
						<p>
							<input type="radio" value="<?php echo esc_html( $key ); ?>" id="wt_iew_export_<?php echo esc_html( $key ); ?>_export" name="wt_iew_export_method_export" <?php checked( $this->export_method, $key ); ?>><b><label for="wt_iew_export_<?php echo esc_html( $key ); ?>_export"><?php echo esc_attr( $value['title'] ); ?></label></b> <br />
			...
			<td colspan="2" style="width:75%;">
				...
				foreach ( $this->mapping_enabled_fields as $mapping_enabled_field_key => $mapping_enabled_field ) {
					...
					<div class="wt_iew_checkbox" style="padding-left:0px;">
						<input type="checkbox" id="wt_iew_<?php echo esc_html( $mapping_enabled_field_key ); ?>" name="wt_iew_include_these_fields[]" value="<?php echo esc_html( $mapping_enabled_field_key ); ?>" <?php checked( $mapping_enabled_field[1], 1 ); ?> /> 
			...
			<td>
				<select class="wt-iew-export-template-sele">
					...
					foreach ( $this->mapping_templates as $mapping_template ) {
						?>
						<option value="<?php echo esc_html( $mapping_template['id'] ); ?>" <?php selected( $form_data_export_template, $mapping_template['id'] ); ?>>
