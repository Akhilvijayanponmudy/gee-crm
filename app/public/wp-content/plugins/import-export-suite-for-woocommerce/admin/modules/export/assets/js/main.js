	...
	var wt_iew_export = {
		...
		set_ajax_page_loader:function (steps, msg_type) {
			if (this.page_overlay) {
				...
				$( '.wt_iew_overlayed_loader' ).show().css( {'height':h,'width':w,'margin-top':'30px','margin-left':'20px'} );
		...
		nonstep_actions:function (action) {
			...
			if (action == 'save_template' || action == 'save_template_as' || action == 'update_template') {
				...
				$( '.wt_iew_template_create_btn' ).unbind( 'click' ).click(
					function () {
		...
		reg_button_actions:function () {
			$( '.wt_iew_export_action_btn' ).unbind( 'click' ).click(
				function (e) {
					e.preventDefault();
					e.stopPropagation();
		...
		prepare_form_data:function () {
			if (this.current_step == 'post_type') {
				...
				$( '.wt_iew_export_step_method_export [name="wt_iew_include_these_fields[]"]' ).each(
					function () {
						if ($( this ).is( ':checked' )) {
				...
				$( '.wt-iew-exporter-default-mapping-tb tbody tr' ).each(
					function () {
				...
				*/
				...
				$( '.wt-iew-exporter-meta-mapping-tb' ).each(
					function () {
		...
		current_step_actions:function () {
			/* current page actions after page is visible */
		...
		page_actions:function (step) {
			if (step == 'post_type') { /* post type page */
				...
				$( '[name="wt_iew_export_method_export"]' ).unbind( 'click' ).click(
					function () {
						var vl = $( this ).val();
				...
				$( '.wt-iew-export-template-sele' ).unbind( 'change' ).change(
					function () {
				...
				if ($( 'select[name="wt_iew_file_into"]' ).length > 0) { /* multiple adapter exists so select box */
					...
					$( '[name="wt_iew_file_into"]' ).unbind( 'change' ).on(
						'change',
						function () {
				...
				$( '.wt_iew_separate_image_export' ).unbind( 'click' ).click(
					function () {
						if ($( this ).val() == 'Yes') {
							$( '.iew_export_image_btn' ).show();
						} else {
				...
				$( '[name="wt_iew_file_as"]' ).unbind( 'change' ).change(
					function () {
		...
		enable_sortable:function () {
			$( '.meta_mapping_box_con[data-sortable="0"]' ).each(
				function () {
		...
		mapping_box_accordian:function () {
			$( '.meta_mapping_box_hd' ).unbind( 'click' ).click(
				function () {
					...
					if (c_dv.is( ':visible' )) {
						...
						c_dv.show();
						c_dv.find( ".wt-iew-mapping-tb tbody tr td" ).each(
							function () {
		...
		reg_mapping_field_bulk_action:function () {
			$( '.wt_iew_mapping_checkbox_main' ).each(
				function () {
			...
			$( '.wt_iew_mapping_checkbox_main' ).unbind( 'click' ).click(
				function () {
					var tb = $( this ).parents( '.wt-iew-mapping-tb' );
					if ($( this ).is( ':checked' )) {
			...
			$( '.wt_iew_mapping_checkbox_sub' ).unbind( 'click' ).click(
				function () {
					var tb = $( this ).parents( '.wt-iew-mapping-tb' );
...
jQuery(
	function () {
		...
		if (wt_iew_export_params.rerun_id > 0) {
			...
			setTimeout(
				function () {
					wt_iew_export.show_step_page( 'filter', true ); },
				1000
			);
		}
