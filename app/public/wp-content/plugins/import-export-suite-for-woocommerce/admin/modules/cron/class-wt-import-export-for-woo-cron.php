...
class Wt_Import_Export_For_Woo_Cron {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'schedule_cron' ), 11 );

		/*
		 * Hook for URL cron (Server cron).
		 */
		add_action( 'init', array( $this, 'do_url_cron' ), 11 );
				...
				if ( $cron_id > 0 ) {
					...
					if ( $cron_data ) {
						if ( isset( $_GET['wt_iew_delete_schedule'] ) ) {
							...
							$history_arr = ( '' !== $cron_data['history_id_list'] ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_data['history_id_list'] ) : array() );
							...
							// Normalize status to integer for strict comparison.
							$cron_status = (int) $cron_data['status'];
							if ( 'enable' === $action ) {
							...
							} elseif ( $cron_status !== self::$status_arr['disabled'] ) {
										$update_data      = array(
	...
	public function advanced_setting_fields( $fields ) {
		$fields['default_time_zone'] = array(
			'label'           => __( 'Switch to website timezone', 'import-export-suite-for-woocommerce' ),
			'type'            => 'checkbox',
			'checkbox_fields' => array( 1 => '' ),
			'value'           => 0,
			'field_name'      => 'default_time_zone',
			'css_class'       => 'wt_iew_checkbox_toggler wt_ier_toggler_blue',
			'help_text'       => __( "Turn on to switch to your wesbite\'s timezone (local timezone). By default the timezone will be in UTC.", 'import-export-suite-for-woocommerce' ),
	...
	public function schedule_now_popup() {
		if ( isset( $_REQUEST['wt_iew_cron_edit_id'] ) && absint( $_REQUEST['wt_iew_cron_edit_id'] ) > 0 ) {
			...
			if ( ! is_null( $cron_module_obj ) ) {
				...
				$cron_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_data['cron_data'] );
	...
	public function enqueue_assets() {
		if ( isset( $_GET['page'] ) ) {
			...
			$wt_time_zone = Wt_Import_Export_For_Woo_Common_Helper::get_advanced_settings( 'default_time_zone' );
	...
	public function do_cron( $action_type, $cron_id = 0 ) {
		...
		if ( ! Wt_Import_Export_For_Woo_Admin::module_exists( $action_modules[ $action_type ] ) ) {
			return;
				...
				$tb          = $wpdb->prefix . Wt_Import_Export_For_Woo::$cron_tb;
				$tme         = Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz( 'now' );
				$is_parallel = 0;
				...
				if ( $cron_list ) {
					...
					foreach ( $cron_list as $cron_listv ) {
						...
						// Get history_id from current cron record (must be inside the loop!).
						$cron_history_id = isset( $cron_listv['history_id'] ) ? absint( $cron_listv['history_id'] ) : 0;

						if ( $cron_history_id > 0 ) {
							// no need to send formdata. It will take from history table by `process_action` method.
							$form_data = array();
						} else {
							$form_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_listv['data'] );
						}

						$cron_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_listv['cron_data'] );
						$file_name = ( isset( $cron_data['file_name'] ) ? $cron_data['file_name'] : '' );
						$cron_status = absint( $cron_listv['status'] );

						$out = null; // Initialize $out to prevent undefined variable warnings.
						if ( $cron_status === self::$status_arr['finished'] || $cron_status === self::$status_arr['not_started'] ) {
							if ( 'import' === $cron_listv['action_type'] ) {
								$out = $action_module->process_download( $form_data, 'download', $cron_listv['item_type'], $cron_history_id, $cron_listv['next_offset'] );
							} else {
								$out = $action_module->process_action( $form_data, $action_type, $cron_listv['item_type'], $file_name, $cron_history_id, $cron_listv['next_offset'] );
							}
						} elseif ( $cron_status === self::$status_arr['running'] ) {
							$out = $action_module->process_action( $form_data, $action_type, $cron_listv['item_type'], $file_name, $cron_history_id, $cron_listv['next_offset'] );
						} elseif ( $cron_status === self::$status_arr['uploading'] ) {
							$out = $action_module->process_upload( 'upload', $cron_history_id, $cron_listv['item_type'] );
						} elseif ( $cron_status === self::$status_arr['downloading'] ) {
							$out = $action_module->process_download( $form_data, $action_type, $cron_listv['item_type'], $cron_history_id, $cron_listv['next_offset'] );
						}//end if

						// Skip processing if $out is not set (invalid status).
						...
						*/
						$new_history_id = isset( $out['history_id'] ) && absint( $out['history_id'] ) > 0 ? absint( $out['history_id'] ) : $cron_history_id;
						...
						$finished = isset( $out['finished'] ) ? absint( $out['finished'] ) : 0;

						if ( ! isset( $out['response'] ) || false === $out['response'] ) {
							...
							if ( 1 === $finished ) {
								...
								// reset the offset.
								// Ensure history_id is updated with the new export_id so upload can find the record.
								$update_data['history_id'] = $new_history_id;
							} elseif ( 3 === $finished ) {
							...
							if ( 0 === $cron_history_id && isset( $out['history_id'] ) && absint( $out['history_id'] ) > 0 ) {
								$history_id_list   = ( '' !== $cron_listv['history_id_list'] ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_listv['history_id_list'] ) : array() );
								$history_id_list   = ( ! is_array( $history_id_list ) ? array() : $history_id_list );
								$history_id_list[] = absint( $out['history_id'] );
	...
	private static function prepare_start_time( $cron_data, $last_start_time = 0 ) {
		...
		} elseif ( 'week' === $cron_data['interval'] ) {
	...
	public function save_schedule( $out ) {
		...
		// process form data.
		$form_data = ( isset( $_POST['form_data'] ) ? Wt_Import_Export_For_Woo_Common_Helper::process_formdata( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( map_deep( wp_unslash( $_POST['form_data'] ), 'sanitize_text_field' ) ) ) : array() ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function edit_schedule( $out ) {
		...
		if ( $cron_details ) {
			$cron_form_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_details['data'] );
			// cron settings data Eg: Cron interval type.
			...
			$update_data                         = array(
				...
				'cron_data'                       => Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_details['cron_data'] ),
			...
			$action_type_base_object = new $action_type_base_class(); // nosemgrep.
	...
	public function update_schedule( $out ) {
		...
		$cron_form_details = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_details['data'] );

		// process form data.
		$form_data = ( isset( $_POST['form_data'] ) ? Wt_Import_Export_For_Woo_Common_Helper::process_formdata( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( map_deep( wp_unslash( $_POST['form_data'] ), 'sanitize_text_field' ) ) ) : array() );
