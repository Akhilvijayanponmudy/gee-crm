...
class Wt_Import_Export_For_Woo_Coupon_Import {
	...
	public function wt_parse_id_field( $data ) {
		...
		if ( $id && 'id' === $this->merge_with ) {
			// Exclude unwanted statuses (trash, auto-draft, inherit) to catch all active posts including WooCommerce orders.
			$id_found_with_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_status NOT IN ( 'trash', 'auto-draft', 'inherit' ) AND ID = %d;", $id ) );
		...
		$code               = isset( $data['code'] ) ? $data['code'] : ( isset( $data['post_title'] ) ? $data['post_title'] : '' );
		$id_found_with_code = '';
	...
	public function set_meta_data( &$object, $data ) {
		if ( isset( $data['meta_data'] ) ) {
			...
			foreach ( $data['meta_data'] as $meta ) {
				...
				// Skip WordPress core meta fields that shouldn't be imported.
				// These are set automatically by WordPress and importing them can cause corruption.
				$skip_core_meta = array( '_edit_lock', '_edit_last', '_wp_old_slug' );
				if ( in_array( $meta_key, $skip_core_meta, true ) ) {
					continue;
				}

				if ( '_used_by' === $meta_key ) {
					$meta_value = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_value );
					// Delete existing _used_by entries for this coupon.
					global $wpdb;
					$wpdb->delete(
						$wpdb->postmeta,
						array(
					...
					// Insert multiple rows (one per user ID).
					foreach ( $meta_value as $user_id ) {
						...
						if ( ! empty( $user_id ) ) {
							if ( 'skip' === $this->found_action ) {
								...
								// Use add_post_meta to create multiple rows (WooCommerce standard).
								add_post_meta( $coupon_id, '_used_by', $user_id );
							}
						}
					}
					// Skip the normal processing for _used_by since we handled it above.
					continue;
				}

				// Unserialize meta value if it's serialized to prevent double serialization.
				// WooCommerce's update_meta_data() will serialize non-scalar values automatically.
			...
			if ( 'skip' === $this->found_action && ! empty( $values ) ) {
				global $wpdb;
				foreach ( $values as $value ) {
					$post_id    = absint( $value['post_id'] );
					$meta_key   = $value['meta_key'];
					$meta_value = $value['meta_value'];

					// Check if value is already serialized (from export) or needs serialization.
