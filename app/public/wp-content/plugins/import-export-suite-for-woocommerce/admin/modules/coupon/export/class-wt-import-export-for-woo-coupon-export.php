	...
	public function generate_row_data( $coupon ) {
		...
		foreach ( $csv_columns as $column => $value ) {
			...
			if ( 'meta:' === substr( $column, 0, 5 ) ) {
				...
				$coupon_meta = get_post_meta( $coupon->ID, $meta, false );
				if ( isset( $coupon_meta ) ) {
					if ( count( $coupon_meta ) > 1 ) {
						$row[ $column ] = maybe_serialize( $coupon_meta );
					} else {
						$row[ $column ] = isset( $coupon_meta[0] ) ? maybe_serialize( $coupon_meta[0] ) : '';
					}
				} else {
					$row[ $column ] = '';
				}
