	...
	public function get_customers_csv_row( $id ) {
		...
		foreach ( $csv_columns as $key => $value ) {
			...
			if ( 'session_tokens' === $key ) {
				$customer_data[ $key ] = ! empty( $user->{$key} ) ? base64_encode( wp_json_encode( Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $user->{$key} ) ) ) : '';
				continue;
			}
