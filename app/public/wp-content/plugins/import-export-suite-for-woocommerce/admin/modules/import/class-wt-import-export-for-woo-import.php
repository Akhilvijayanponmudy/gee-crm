...
class Wt_Import_Export_For_Woo_Import {
	...
	/**
	 * Fast import mode - Enable this in general settings for removing do_action while wp_insert_post
	 *
	 * @var bool
	 */
	public $fast_import_mode = false;
	/**
	 * Module ID
	 *
	 * @var string
	...
	public function get_defaults() {
		...
		$this->fast_import_mode       = 1 === absint( $import_speed_mode );
	}
	...
	public function advanced_setting_fields( $fields ) {
				...
				$fields['enable_speed_mode']      = array(
					'label'           => __( 'Block third party plugin hooks while importing', 'import-export-suite-for-woocommerce' ),
					'type'            => 'checkbox',
					'checkbox_fields' => array( 1 => __( 'Enable', 'import-export-suite-for-woocommerce' ) ),
					'value'           => 0,
					'field_name'      => 'enable_speed_mode',
					'field_group'     => 'advanced_field',
					'help_text'       => __( 'Increase speed by disabling do_action calls in wp_insert_post during import. Verify your imported posts work properly if you enabled this.', 'import-export-suite-for-woocommerce' ),
					'validation_rule' => array( 'type' => 'absint' ),
				);
			...
			if ( $response['response'] ) {
				...
				if ( count( $temp_files ) > 1 ) {
					foreach ( $temp_files as $key => $temp_file ) {
						if ( basename( $temp_file ) !== $this->temp_import_file && file_exists( $temp_file ) ) {
							// nosemgrep.
		...
		if ( $rerun_id > 0 ) {
			...
			if ( ! is_null( $history_module_obj ) ) {
				...
				if ( $history_data && $history_data['template_type'] === $this->module_base ) {
					$form_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $history_data['data'] );
		...
		if ( $rerun_id > 0 ) {
			...
			if ( ! is_null( $cron_module_obj ) ) {
				...
				if ( $cron_data && $cron_data['action_type'] === $this->module_base ) {
					$form_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_data['data'] );
					if ( $form_data && is_array( $form_data ) ) {
						$this->to_import = ( isset( $form_data['post_type_form_data'] ) && isset( $form_data['post_type_form_data']['item_type'] ) ? $form_data['post_type_form_data']['item_type'] : '' );
						if ( '' !== $this->to_import ) {
							$this->import_method = ( isset( $form_data['method_import_form_data'] ) && isset( $form_data['method_import_form_data']['method_import'] ) && '' !== $form_data['method_import_form_data']['method_import'] ? $form_data['method_import_form_data']['method_import'] : $this->default_import_method );
		...
		if ( Wt_Import_Export_For_Woo_Common_Helper::wt_is_screen_allowed() ) {
			...
			$params = array(
				...
				'msgs'                          => array(
					...
					'no_columns_mapped'         => __( 'No columns have been mapped. Please map the required fields to proceed with the import.', 'import-export-suite-for-woocommerce' ),
				),
			...
			if ( '' !== $file_url ) {
				...
				if ( $ext && $this->is_extension_allowed( $file_url ) ) {
					...
					if ( $file_path ) {
						...
						if ( ! is_wp_error( $file_data ) && 200 === wp_remote_retrieve_response_code( $file_data ) ) {
	...
	public function process_action( $form_data, $step, $to_process, $file_name = '', $import_id = 0, $offset = 0 ) {
		...
		if ( $import_id > 0 ) {
			...
			$form_data = ( isset( $import_data['data'] ) ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $import_data['data'] ) : array() );
		...
		*/
		...
		// Check if speed mode is enabled for this import.
		$speed_mode_value = isset( $form_data['advanced_form_data']['wt_iew_enable_speed_mode'] ) ? $form_data['advanced_form_data']['wt_iew_enable_speed_mode'] : null;
		$fast_mode         = ( 1 === absint( $speed_mode_value ) ) ? true : $this->fast_import_mode;

		if ( $fast_mode ) {
			/**
			 * Filter the list of post update actions to disable during fast import mode.
			 *
			 * @since 1.4.0
			 *
			 * @param array $actions Array of action names to disable.
			 */
			$actions = apply_filters(
				'wt_iew_disable_post_update_actions',
				array(
	...
	public function ajax_main() {
		...
		if ( Wt_Iew_Sh::check_write_access( WT_IEW_PLUGIN_ID ) ) {
			...
			*/
			...
			if ( 'json' === $data_type ) {
				/**
				 * 1.3.3 - delete the option after each batch.
				 */
