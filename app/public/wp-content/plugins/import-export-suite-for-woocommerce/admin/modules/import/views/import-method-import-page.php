			...
			<td colspan="2" style="width:75%;">
				<div class="wt_iew_radio_block">
					...
					foreach ( $this->import_obj->import_methods as $key => $value ) {
						?>
						<p>
							<input type="radio" value="<?php echo esc_html( $key ); ?>" id="wt_iew_import_<?php echo esc_html( $key ); ?>_import" name="wt_iew_import_method_import" <?php checked( $this->import_method, $key ); ?>><b><label for="wt_iew_import_<?php echo esc_html( $key ); ?>_import"><?php echo esc_html( $value['title'] ); ?></label></b> <br />
			...
			<td>
				<select class="wt-iew-import-template-sele">
					...
					foreach ( $this->mapping_templates as $mapping_template ) {
						?>
						<option value="<?php echo esc_html( $mapping_template['id'] ); ?>" <?php selected( $form_data_import_template, $mapping_template['id'] ); ?>>
