var wt_iew_import=(function( $ ) {
	...
	var wt_iew_import=
	{
		...
		{
		    	if(typeof heavy_meta !== 'undefined'){
			...
			this.prepare_ajax_data('get_meta_mapping_fields', 'json');
                        if(typeof wt_iew_file_head_remaining_meta !== 'undefined'){
					...
					if (!wt_iew_import.import_finished) {
						...
						if(!wt_iew_import.import_cancel_clicked) {
							...
							if (-1 !== jQuery.inArray(jqXHR.status, timeoutStatus)) {
								...
								jQuery('.resume-import-button').off('click').on('click', function (event) {
			 ...
			 */ 
				...
				if(!this.is_previous_step(step_to_go))
				{
					...
					$default_table.find('tbody tr').each(function(){
						...
						var val = $row.find('.columns_val').val();
						var has_value = (val ? String(val).trim() : '') !== '';
					...
					{
						wt_iew_notify_msg.error(wt_iew_import_params.msgs.no_columns_mapped);
						return false;
					}
				}	
			}
		...
		prepare_ajax_data:function(action, data_type)
		{
	            ...
	            'temp_import_file': this.temp_import_file,
                    ...
                    if($('[name="wt_iew_delimiter_preset"]').length>0){
                        this.ajax_data['delimiter_preset'] = $('[name="wt_iew_delimiter_preset"]').val();
                    }                    
	...
	var mapping_field_editor=
	{
		...
		do_arithmetic:function(str)
		{
			...
			while ((m = regex_arith.exec(str)) !== null) {
			    ...
			    try{
			    	eqn='('+m[1]+')';
			        eval("var eqn_eval = " + eqn.toLowerCase()); // nosemgrep
