...
class Wt_Import_Export_For_Woo_FtpAdapter extends Wt_Import_Export_For_Woo_RemoteAdapter {
	...
	public function test_ftp( $profile_id, $ftp_profile = array() ) {
		...
		// Normalize is_sftp to integer for strict comparison (database may return string).
		$is_sftp = isset( $ftp_profile['is_sftp'] ) ? (int) $ftp_profile['is_sftp'] : 0;
		if ( 1 === $is_sftp ) {
			...
			if ( 'implicit_ftp_connection' === $ftp_conn ) {
				...
				$out['status'] = true;
			} elseif ( false !== $ftp_conn && 'implicit_ftp_connection' !== $ftp_conn && ( is_resource( $ftp_conn ) || ( is_object( $ftp_conn ) && $ftp_conn instanceof \FTP\Connection ) ) ) {
				// successfully connected - verify it's a valid FTP connection resource.
					$login = @ftp_login( $ftp_conn, $ftp_profile['user_name'], $ftp_profile['password'] ); // nosemgrep.
				if ( $login ) {
					...
					if ( 1 === $ftp_profile['passive_mode'] ) {
						if ( ! @ftp_pasv( $ftp_conn, true ) ) {
							...
							if ( is_resource( $ftp_conn ) || ( is_object( $ftp_conn ) && $ftp_conn instanceof \FTP\Connection ) ) {
								@ftp_close( $ftp_conn ); // nosemgrep.
							}
							return $out;
						} else {
							$out['msg']    = __( 'Successfully tested.' );
				...
				// Close FTP connection if it's a valid resource.
				if ( is_resource( $ftp_conn ) || ( is_object( $ftp_conn ) && $ftp_conn instanceof \FTP\Connection ) ) {
					@ftp_close( $ftp_conn ); // nosemgrep.
				}
			} else {
	...
	public function download( $form_data, $out, $import_obj ) {
		...
		if ( 1 === $is_sftp ) {
			...
			if ( 'implicit_ftp_connection' === $ftp_conn ) {
				...
				// Close FTP connection before returning (handles both successful and failed login).
				if ( is_resource( $ftp_conn ) || ( is_object( $ftp_conn ) && $ftp_conn instanceof \FTP\Connection ) ) {
					@ftp_close( $ftp_conn ); // nosemgrep.
				}
			} else {
	...
	protected function connect_ftp( $ftp_profile ) {
		if ( 1 === (int) $ftp_profile['ftps'] ) {
			// use ftps.
			$ftp_conn = @ftp_ssl_connect( $ftp_profile['server'], $ftp_profile['port'] ); // nosemgrep.
		} else {
			$ftp_conn = @ftp_connect( $ftp_profile['server'], $ftp_profile['port'] ); // nosemgrep.
		}
		...
		if ( empty( $ftp_conn ) ) {
			...
			// SSL verification disabled here only to allow connecting to legacy FTPS endpoints that use self-signed or mismatched certs.
			// nosemgrep.
	...
	public function upload( $local_file, $remote_file_name, $form_data, $out ) {
		...
		if ( 1 === $is_sftp ) {
			...
			if ( 'implicit_ftp_connection' === $ftp_conn ) {
			...
			} elseif ( 22 === (int) $ftp_profile['port'] && 0 === (int) $ftp_profile['is_sftp'] ) {
				$out['msg'] = sprintf(
					/* translators: 1: Server address, 2: Port number */
					__( 'Port 22 is typically used for SFTP, not FTP. Please use SFTP option instead, or use FTP on port 21. Failed to connect to %1$s:%2$d.', 'import-export-suite-for-woocommerce' ),
					$ftp_profile['server'],
					$ftp_profile['port']
				);
			} else {
				$out['msg'] = sprintf(
					/* translators: 1: Server address, 2: Port number */
					__( 'Failed to establish FTP connection to %1$s:%2$d. Please check your server address, port, and network connectivity.', 'import-export-suite-for-woocommerce' ),
					$ftp_profile['server'],
					$ftp_profile['port']
				);
			}
	...
	private function wt_implicit_ftp_file_upload( $local, $remote, $server, $username, $password ) {
		...
		if ( $fp ) { // @codingStandardsIgnoreLine.
			...
			// SSL verification disabled to support legacy implicit FTPS servers with self-signed certs.
			// nosemgrep.
