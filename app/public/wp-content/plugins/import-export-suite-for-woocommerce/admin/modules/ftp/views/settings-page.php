		...
		<form method="post" action="<?php echo esc_url( $req_url ); ?>" id="wt_iew_ftp_form">
			...
			<table class="form-table wt-iew-form-table">
				...
				<tr>
					...
					<?php } else { ?>
					<td>
						<div class="wt-iew-sftp-notice">
							<p><?php esc_html_e( 'SFTP Addon not found.' ); ?></p>
						</div>
					</td>					
					<?php } ?>
					<td></td>
			...
			$settings_button_title = __( 'Save settings', 'import-export-suite-for-woocommerce' );
			$before_button_text    = ( 1 === $this->popup_page ? '<span style="line-height:40px;"><input type="checkbox" id="wt_iew_add_and_use_ftp" name="wt_iew_add_and_use_ftp" value="1"> <label for="wt_iew_add_and_use_ftp">' . esc_html__( 'Use this profile and close the popup', 'import-export-suite-for-woocommerce' ) . '</label></span>' : '' );
			$after_button_text     = '<input type="button" name="wt_iew_abort_test_ftp" value="' . __( 'Abort FTP testing', 'import-export-suite-for-woocommerce' ) . '" class="button button-secondary wt_iew_abort_test_ftp" style="float:right; margin-right:10px; display:none;" />';
			$after_button_text    .= '<input type="button" name="wt_iew_test_ftp_form" value="' . __( 'Test FTP', 'import-export-suite-for-woocommerce' ) . '" class="button button-secondary wt_iew_test_ftp_form" style="float:right; margin-right:10px;" />';
