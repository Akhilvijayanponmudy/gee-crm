...
class Wt_Import_Export_For_Woo_Subscription_Import {
	...
	public function parse_subscription_orders( $item, $merge ) {
		try {
			...
			if ( self::$is_hpos_enabled && $subscription_id ) {
				...
				if ( $result_query && 'skip' === $this->id_conflict ) {
					...
					unset( $result['subscription_id'] );
				}
			} elseif ( $subscription_id && is_string( get_post_status( $subscription_id ) ) && ( 'shop_subscription' !== get_post_type( $subscription_id ) ) && ( 'hf_shop_subscription' !== get_post_type( $subscription_id ) ) && ( 'shop_order_placehold' ) !== get_post_type( $subscription_id ) ) {
				if ( 'skip' === $this->id_conflict ) {
			...
			if ( ! empty( $data['tax_items'] ) ) {
				...
				foreach ( $tax_item as $tax ) {
					...
					if ( ( $tax_item_data['rate_id'] && ! isset( $tax_rates[ $tax_item_data['rate_id'] ] ) ) || 0 === $tax_item_data['rate_id'] ) {
						$tax_item_data['rate_id'] = 0;
					} elseif ( isset( $tax_item_data['label'] ) && ( ( 0 !== strcasecmp( $tax_rates[ $tax_item_data['rate_id'] ]->tax_rate_name, $tax_item_data['label'] ) ) || ( $tax_rates[ $tax_item_data['rate_id'] ]->tax_rate !== $tax_item_data['rate_percent'] ) ) ) {
			...
			if ( ! empty( $data['order_items'] ) ) {
				...
				foreach ( $_order_items as $item ) {
					...
					$tax_data = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $tax_data );
					if ( ! empty( $tax_data ) ) {
						$tax_data     = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $tax_data );
						$new_tax_data = array();
		...
		try {
			...
			if ( ! empty( $data['renewal_orders'] ) ) {
				...
				if ( ! empty( $current_order_ids ) && ! class_exists( 'HF_Subscription' ) ) {
					update_option( '_transient_wcs-related-orders-to-' . ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), $current_order_ids );
				}
				foreach ( $current_order_ids as $id ) {
					if ( self::$is_hpos_enabled ) {
						$order = WC()->order_factory->get_order( $id );
						$order->update_meta_data( '_subscription_renewal', ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id );
					}
					if ( false !== strpos( self::$table_name, 'posts' ) || self::$is_sync ) {
						update_post_meta( $id, '_subscription_renewal', ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id );
					}
				}
			}//end if
			...
			foreach ( $data['post_meta'] as $meta_data ) {
				switch ( $meta_data['key'] ) {
					...
					case '_download_permissions':
						...
						if ( false !== strpos( self::$table_name, 'posts' ) || self::$is_sync ) {
							update_post_meta( ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), '_download_permissions_granted', $meta_data['value'] );
						}
						break;
					case '_requires_manual_renewal':
						// Skip processing _requires_manual_renewal from CSV if we have a payment method in the data,
						// as it will be set correctly based on payment gateway support in subscription class.
						// Only process it if payment method is empty or manual.
						$payment_method_from_data = isset( $data['payment_method'] ) ? $data['payment_method'] : '';
						if ( empty( $payment_method_from_data ) || 'manual' === $payment_method_from_data ) {
							$subscription->set_requires_manual_renewal( wc_string_to_bool( $meta_data['value'] ) );
						}
						break;
					case '_order_currency':
						// Use setter method instead of update_meta_data for internal meta keys.
						if ( is_callable( array( $subscription, 'set_currency' ) ) ) {
							$subscription->set_currency( $meta_data['value'] );
						}
						break;
					default:
						if ( self::$is_hpos_enabled ) {
						...
						if ( false !== strpos( self::$table_name, 'posts' ) || self::$is_sync ) {
							update_post_meta( ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), $meta_data['key'], $meta_data['value'] );
						}
				}
			}
			if ( self::$is_hpos_enabled ) {
				...
				if ( ! empty( $order_operational_data ) ) {
					$shipping_tax_amount   = $order_operational_data['shipping_tax_amount'] ? $order_operational_data['shipping_tax_amount'] : 0;
					$shipping_total_amount = $order_operational_data['shipping_total_amount'] ? $order_operational_data['shipping_total_amount'] : 0;
					$discount_tax_amount   = $order_operational_data['discount_tax_amount'] ? $order_operational_data['discount_tax_amount'] : 0;
					$discount_total_amount = $order_operational_data['discount_total_amount'] ? $order_operational_data['discount_total_amount'] : 0;
					$order_id              = ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id;
					$wpdb->query( $wpdb->prepare( " UPDATE {$wpdb->prefix}wc_order_operational_data SET shipping_tax_amount= %f,shipping_total_amount=%f,discount_tax_amount=%f,discount_total_amount=%f WHERE order_id=%d", $shipping_tax_amount, $shipping_total_amount, $discount_tax_amount, $discount_total_amount, $order_id ) );
				}
			}
			...
			if ( ! empty( $data['order_items'] ) ) {
				$wpdb->query( $wpdb->prepare( "DELETE items,itemmeta FROM {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items ON itemmeta.order_item_id = items.order_item_id WHERE items.order_id = %d and items.order_item_type = 'line_item'", ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ) );
				if ( is_numeric( $data['order_items'] ) ) {
					...
					if ( $add_memberships ) {
						self::maybe_add_memberships( $user_id, ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), $product_id );
					}
				} else {
					foreach ( $data['order_items'] as $order_item ) {
						...
						if ( $add_memberships ) {
							self::maybe_add_memberships( $user_id, ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), $item_data['product_id'] );
						}
					}
				}
			}//end if
			...
			if ( ! empty( $data['shipping_method'] ) ) {
				$wpdb->query( $wpdb->prepare( "DELETE items,itemmeta FROM {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items ON itemmeta.order_item_id = items.order_item_id WHERE items.order_id = %d and items.order_item_type = 'shipping'", ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ) );
				...
				$tax_data      = Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $taxes );
				if ( isset( $tax_data['total'] ) ) {
					$new_tax_data = array();
					foreach ( $tax_data['total'] as $t_key => $t_value ) {
				...
				$shipping_order_item_id = wc_add_order_item( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id, $shipping_order_item );
			...
			if ( ! empty( $data['fee_items'] ) ) {
				$fee_str = 'fee';
				$wpdb->query( $wpdb->prepare( "DELETE items,itemmeta FROM {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d and items.order_item_type = %s", ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id, $fee_str ) );
				...
				foreach ( $fee_items as $item ) {
					...
					$fee_order_item_id = wc_add_order_item( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id, $fee_order_item );
			...
			if ( ! empty( $data['tax_items'] ) ) {
				$tax_str = 'tax';
				$wpdb->query( $wpdb->prepare( "DELETE items,itemmeta FROM {$wpdb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d and items.order_item_type = %s", ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id, $tax_str ) );
				foreach ( $data['tax_items'] as $tax_item ) {
					$tax_order_item    = array(
						'order_item_name' => $tax_item['title'],
						'order_item_type' => 'tax',
					);
					$tax_order_item_id = wc_add_order_item( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id, $tax_order_item );
			...
			if ( ! empty( $data['coupon_items'] ) ) {
				if ( $merge && $is_order_exist ) {
					$applied_coupons = is_callable( array( $subscription, 'get_coupon_codes' ) ) ? $subscription->get_coupon_codes() : $subscription->get_used_coupons();
			...
			if ( ! empty( $data['order_notes'] ) ) {
				add_filter( 'woocommerce_email_enabled_customer_note', '__return_false' );
					$wpdb->query( $wpdb->prepare( "DELETE comments,meta FROM {$wpdb->prefix}comments comments LEFT JOIN {$wpdb->prefix}commentmeta meta ON comments.comment_ID = meta.comment_id WHERE comments.comment_post_ID = %d", ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ) );
			...
			$this->hf_log_data_change( 'hf-subscription-csv-import', sprintf( __( '> &#8220;%1$s&#8221; %2$s' ), esc_html( $subscription_id ), $out_msg ), true );
			// $this->imported++;
			/* translators:%s: subscription number */
			$this->hf_log_data_change( 'hf-subscription-csv-import', sprintf( __( '> Finished importing subscription %s' ), $subscription_id ) );
			...
			if ( $this->delete_existing ) {
				update_post_meta( ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), '_wt_delete_existing', 1 );
			}

			return array( 'id' => ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ) );
		} catch ( Exception $e ) {
			return new WP_Error( 'woocommerce_product_importer_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}//end try
	...
	public function hf_check_customer( $data, $email_customer = false ) {
		...
		$subscription_id = isset( $data['subscription_id'] ) ? $data['subscription_id'] : '';
		$order_exist    = ! empty( $subscription_id ) ? $this->subscription_order_exists( $subscription_id ) : false;
	...
	public function hf_create_subscription( $args = array(), $subscription_exist = false ) {
		...
		if ( $subscription_exist ) {
			...
			if ( ! $check_post_table ) {
				$check_post_table = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type= %s AND ID=%d", 'shop_order_placehold', $args['subscription_id'] ) );
			}
			...
			if ( isset( $check_order_table ) && ! $check_post_table ) {
			...
			} elseif ( ! isset( $check_order_table ) && $check_post_table ) {
				if ( self::$is_hpos_enabled ) {
				...
				if ( self::$is_hpos_enabled && ( self::$is_sync || class_exists( 'HF_Subscription' ) ) ) {
					$result_post_table = $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->posts} (ID, post_status, post_date, post_date_gmt, post_type) VALUES (%d, 'wc-pending', %s, %s, %s)", $import_id, $current_date, $current_date, $post_type ) );
				} elseif ( self::$is_hpos_enabled && ! self::$is_sync ) {
	...
	public static function add_product( $details, $subscription, $data, $link_product_using_sku ) {
		...
		if ( ! $_product ) {
			...
			$product_id = ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $_product->get_id() : $_product->id;
			...
			$product_string = sprintf( '<a href="%s">%s</a>', get_edit_post_link( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $product_id : $_product->id ), $line_item_name );
		...
		$order_item_id = wc_add_order_item( ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $subscription->get_id() : $subscription->id ), $order_item );

		if ( $order_item_id ) {
			foreach ( $_order_item_meta as $meta_key => $meta_value ) {
				wc_add_order_item_meta( $order_item_id, $meta_key, Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $meta_value ) );
			}
		}
	...
	public static function save_download_permissions( $subscription, $product, $quantity = 1 ) {
		if ( $product && $product->exists() && $product->is_downloadable() ) {
			...
			$product_id = isset( $product->variation_id ) ? $product->variation_id : ( ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $product->get_id() : $product->id );
		...
		if ( ! empty( $coupon_items ) ) {
			foreach ( $coupon_items as $coupon_item ) {
				...
				if ( ! $coupon ) {
					...
					$discount_amount = ( version_compare( WC()->version, '2.7.0', '>=' ) ) ? $coupon->get_amount() : $coupon->discount_amount;
				}
	...
	public function data_validation( $args = array(), $subscription_exist = false ) {
		...
		} elseif ( strtotime( $args['start_date'] ) > time() ) {
			if ( ! $subscription_exist ) {
