	...
	?>
			...
			foreach ( $summary['type'] as $summary_row ) {
				...
				if ( $summary_row_count ) :
					$summary_row_description = isset( $summary_row['description'] ) ? $summary_row['description'] : __( 'Unknown error', 'import-export-suite-for-woocommerce' );
					$summary_row_help_link   = ( ! empty( $summary_item ) && isset( $summary['items'][ $summary_item ] ) ) ? $summary['items'][ $summary_item ] : '';

					if ( ! empty( $summary_row_help_link ) ) {
						?>
						<p><?php echo esc_html( $summary_row_description . "($summary_row_count)" ); ?> - <?php esc_html_e( 'Please refer', 'import-export-suite-for-woocommerce' ); ?> <a href="<?php echo esc_url( $summary_row_help_link ); ?>" target="_blank"><?php esc_html_e( 'to this article', 'import-export-suite-for-woocommerce' ); ?></a> <?php esc_html_e( 'for troubleshoot.', 'import-export-suite-for-woocommerce' ); ?></p> 
						<?php
					} else {
						?>
						<p><?php echo esc_html( $summary_row_description . "($summary_row_count)" ); ?></p>
						<?php
					}
				endif;
			}
