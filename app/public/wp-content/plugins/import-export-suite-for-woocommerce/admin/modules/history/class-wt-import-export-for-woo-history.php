...
class Wt_Import_Export_For_Woo_History {
		...
		if ( $history_id > 0 ) {
			...
			if ( $history_item ) {
				...
				if ( 'import' === $action_type && Wt_Import_Export_For_Woo_Admin::module_exists( $action_type ) ) {
					...
					// Log file path is derived from plugin-managed identifiers (not arbitrary input).
					// nosemgrep.
					if ( file_exists( $log_file_path ) ) {
						...
						if ( $response['response'] ) {
							...
							ob_start();
							// View file comes from plugin directory, not user input.
							// nosemgrep.
		...
		} else // raw log viewing.
		{
			...
			if ( '' !== $log_file_name ) {
				...
				if ( 'log' === $ext ) {
					...
					// File path resolved only for .log files within plugin-managed directory.
					// nosemgrep.
					if ( file_exists( $log_file_path ) ) {
						include_once WT_IEW_PLUGIN_PATH . 'admin/classes/class-wt-import-export-for-woo-logreader.php';
						$reader   = new Wt_Import_Export_For_Woo_Logreader();
				...
				if ( is_array( $log_file_arr ) ) {
					foreach ( $log_file_arr as $log_file_name ) {
						if ( '' !== $log_file_name ) {
							...
							if ( 'log' === $ext ) {
								$log_file_path = Wt_Import_Export_For_Woo_Log::get_file_path( $log_file_name );
								// Only deletes .log files resolved inside plugin log directory.
								// nosemgrep.
	...
	public function admin_settings_page() {
		...
		if ( $cron_id > 0 ) {
			...
			if ( ! is_null( $cron_module_obj ) ) {
				...
				if ( $cron_data ) {
					$history_id_arr = ( '' !== $cron_data['history_id_list'] ? Wt_Import_Export_For_Woo_Common_Helper::wt_unserialize_safe( $cron_data['history_id_list'] ) : array() );
	...
	public static function delete_history_by_id( $id ) {
		...
		if ( $list ) {
			foreach ( $list as $listv ) {
				...
				} elseif ( 'import' === $listv['template_type'] ) {
	...
	public static function auto_delete_history_entry( $record_count = 0 ) {
		if ( 0 === $record_count ) {
			...
			if ( 1 === Wt_Import_Export_For_Woo_Common_Helper::get_advanced_settings( 'enable_history_auto_delete' ) ) {
			...
			if ( Wt_Iew_Sh::check_write_access( WT_IEW_PLUGIN_ID ) ) {
				...
				if ( '' !== $file_name ) {
					...
					if ( 'log' === $file_ext ) {
						...
						// Path is constrained to plugin log directory after .log extension check.
						// nosemgrep.
