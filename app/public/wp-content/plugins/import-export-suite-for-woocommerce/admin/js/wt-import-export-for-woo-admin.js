...
var wt_iew_settings_form =
{
	Set:function (prnt_selector) {
		...
		wf_prnt_obj.find( '.wt_iew_settings_form' ).submit(
			function (e) {
				e.preventDefault();
...
var wt_saved_templates = {
	Set: function () {
				jQuery( '.wt_ier_delete_template' ).unbind( 'click' ).click(
					function (e) {

						e.preventDefault();
						if (confirm( wt_iew_params.msgs.sure )) {
							...
							jQuery( 'tr[data-row-id=' + data.template_id + ']' ).html( '<td colspan="5">' + wt_iew_params.msgs.template_del_loader + '</td>' );
							jQuery.ajax(
								{
									url: wt_iew_params.ajax_url,
									type: 'POST',
									...
									error: function () {
										wt_iew_notify_msg.error( wt_iew_params.msgs.template_del_error );
									}
								}
							);
						}
...
wt_iew_popup = {
	...
	regPopupOpen:function () {
		jQuery( '[data-wt_iew_popup]' ).click(
			function () {
	...
	regPopupClose:function (popup_elm) {
		...
		jQuery( '.wt_iew_popup_close, .wt_iew_popup_cancel' ).unbind( 'click' ).click(
			function () {
...
var wt_iew_form_toggler =
{
	Set:function () {
		this.runToggler();
		jQuery( 'select.wt_iew_form_toggler' ).change(
			function () {
				wt_iew_form_toggler.toggle( jQuery( this ) );
			}
		);
		jQuery( 'input[type="radio"].wt_iew_form_toggler' ).click(
			function () {
		...
		jQuery( 'input[type="checkbox"].wt_iew_form_toggler' ).click(
			function () {
	...
	runToggler:function (prnt) {
		...
		prnt.find( 'select.wt_iew_form_toggler' ).each(
			function () {
				wt_iew_form_toggler.toggle( jQuery( this ) );
			}
		);
		prnt.find( 'input[type="radio"].wt_iew_form_toggler, input[type="checkbox"].wt_iew_form_toggler' ).each(
			function () {
				if (jQuery( this ).is( ':checked' )) {
					wt_iew_form_toggler.toggle( jQuery( this ) );
				}
		...
		prnt.find( 'input[type="checkbox"].wt_iew_form_toggler' ).each(
			function () {
				wt_iew_form_toggler.toggle( jQuery( this ),1 );
			}
		);
	...
	toggle:function (elm, checkbox) {
		var vl   = elm.val();
		var trgt = elm.attr( 'wt_iew_frm_tgl-target' );
		jQuery( '[wt_iew_frm_tgl-id="' + trgt + '"]' ).hide();
		if ( ! elm.is( ':visible' )) {
			return false;
		}
		if (elm.css( 'display' ) != 'none') { /* if parent is visible. `:visible` method. it will not work on JS tabview */
			...
			elms.each(
				function () {
					var lvl   = jQuery( this ).attr( 'wt_iew_frm_tgl-lvl' );
					var mrgin = 25;
		...
		jQuery( '[wt_iew_frm_tgl-id="' + trgt + '"]' ).each(
			function () {
	...
	getElms:function (elm,trgt,vl,checkbox) {
		...
		return jQuery( '[wt_iew_frm_tgl-id="' + trgt + '"]' ).filter(
			function () {
				if (jQuery( this ).attr( 'wt_iew_frm_tgl-val' ) == vl) {
					if (checkbox) {
						if (elm.is( ':checked' )) {
							if (jQuery( this ).attr( 'wt_iew_frm_tgl-chk' ) == 'true') {
								return true;
							} else {
								return false;
							}
						} else {
							if (jQuery( this ).attr( 'wt_iew_frm_tgl-chk' ) == 'false') {
								return true;
							} else {
								return false;
							}
...
var wt_iew_file_attacher = {
	...
	Set:function () {
		var file_frame;
		jQuery( ".wt_iew_file_attacher" ).click(
			function (event) {
				...
				if (jQuery( this ).data( 'file_frame' )) {
					...
					jQuery( this ).data( 'file_frame' ).on(
						'select',
						function () {
...
var wt_iew_popover =
{
	action_module:false,
	Set:function () {
		jQuery( '[data-wt_iew_popover="1"]' ).unbind( 'click' ).click(
			function () {
				...
				pp_elm.css( {'display':'block','opacity':0, 'top':cr_elm_pos_t,'left':cr_elm_pos_l} ).stop( true,true ).animate(
					{'left':cr_elm_pos_l + 20,'opacity':1},
					500,
					function () {
	...
	do_action:function () {
		jQuery( '[name="wt_iew_popover_do_action"]' ).unbind( 'click' ).click(
			function () {
	...
	regclosePop:function () {
		jQuery( '.meta_mapping_box_toggle' ).click(
			function () {
				wt_iew_popover.closePop();
			}
		);
		jQuery( '.popover_close' ).unbind( 'click' ).click(
			function () {
				wt_iew_popover.closePop();
			}
		);
	...
	closePop:function () {
		...
		if (pp_elm.length > 0) {
			...
			jQuery( '[data-wt_iew_popover="1"]' ).attr( 'data-popup-opened',0 );
			pp_elm.stop( true,true ).animate(
				{'opacity':0, 'left':pp_lft},
				300,
				function () {
...
var wt_iew_custom_and_preset     =
{
	toggler:function (preset_elm, custom_elm, custom_val) {
		...
		preset_elm.unbind( 'change' ).change(
			function () {
	...
	delimiter_toggler:function () {
		/* function for delimiter toggle */
		this.toggler( jQuery( '.wt_iew_delimiter_preset' ), jQuery( '.wt_iew_custom_delimiter' ), 'other' );
	},
	date_format_toggler:function () {
		/* function for date format toggle */
...
var wt_iew_conditional_help_text =
{
	Set:function (prnt) {
		...
		const regex = /\[(.*?)\]/gm;
		let m;
		prnt.find( '.wt-iew_conditional_help_text' ).each(
			function () {
