...
foreach ( $form_fields as $key => $value ) {
	...
	$html_id           = ( isset( $value['html_id'] ) ? ' id="' . esc_attr( $value['html_id'] ) . '" ' : '' );
	...
	*/
	...
	if ( isset( $value['form_toggler'] ) ) {
		if ( 'parent' === $value['form_toggler']['type'] ) {
			...
			$form_toggler_register = ' wt_iew_frm_tgl-target="' . esc_attr( $value['form_toggler']['target'] ) . '"';
	...
	$css_attr   = ( '' !== $css_class ? ' class="' . esc_attr( $css_class ) . '"' : '' );
			...
			<td <?php echo wp_kses_post( $colspan_attr ); ?> class="<?php echo wp_kses_post( $td_class2 ); ?>">
				...
				if ( '' === $field_html ) {
					...
					if ( 'text' === $form_element_type || 'number' === $form_element_type || 'password' === $form_element_type ) {
						?>
						<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $html_id, $css_attr, $attr are already escaped with esc_attr(). ?>
						<input placeholder="<?php echo esc_attr( $placeholder ); ?>" type="<?php echo esc_attr( $form_element_type ); ?>" <?php echo $html_id; ?> <?php echo $css_attr; ?> name="<?php echo esc_attr( $form_data_key ); ?>" value="<?php echo esc_attr( $vl ); ?>" <?php echo $attr; ?> >
						<?php
					}
					if ( 'textarea' === $form_element_type ) {
						...
						<div class="wt_form_radio_block">
							...
							/* Normalize $vl once before loop to ensure type consistency for checked() comparison */
							$vl_normalized = (string) $vl;
							foreach ( $radio_fields as $rad_vl => $rad_label ) {
								?>
									<span style="display:inline-block;">
										<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $css_attr, $attr, $form_toggler_register are already escaped with esc_attr(). ?>
										<input <?php echo $css_attr; ?> type="radio" id="<?php echo esc_attr( $form_data_key . '_' . $rad_vl ); ?>" name="<?php echo esc_attr( $form_data_key ); ?>" value="<?php echo esc_attr( $rad_vl ); ?>" <?php checked( $vl_normalized, $rad_vl ); ?> <?php echo $attr; ?> <?php echo $form_toggler_register; ?> /> <?php echo wp_kses_post( $rad_label ); ?>
									</span>
														&nbsp;&nbsp;
								<?php
							}
						...
						<div class="wt_form_radio_block">
							...
							foreach ( $radio_fields as $rad_vl => $rad_label ) {
								?>
							<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $css_attr is already escaped with esc_attr() on line 119. ?>
							<input <?php echo $css_attr; ?> type="radio" id="<?php echo esc_attr( $form_data_key . '_' . $rad_vl ); ?>" name="<?php echo esc_attr( $form_data_key ); ?>" value="<?php echo esc_attr( $rad_vl ); ?>" <?php checked( $vl, $rad_vl ); ?> <?php echo $attr; ?> <?php echo $form_toggler_register; ?> /> <?php echo wp_kses_post( $rad_label ); ?>
							&nbsp;&nbsp;
								<?php
							}
							?>
						...
						<div class="wt_form_checkbox_block">
							...
							foreach ( $checkbox_fields as $chk_vl => $chk_label ) {
								$chk_vl_normalized = (string) $chk_vl;
								?>
									<span style="display:inline-block;">
										<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $css_attr, $attr, $form_toggler_register are already escaped with esc_attr(). ?>
										<input <?php echo $css_attr; ?> type="checkbox" id="<?php echo esc_attr( $form_data_key . '_' . $chk_vl ); ?>" name="<?php echo esc_attr( $form_data_key ); ?>" value="<?php echo esc_attr( $chk_vl ); ?>" <?php checked( $vl_normalized, $chk_vl_normalized ); ?> <?php echo $attr; ?> <?php echo $form_toggler_register; ?> /> <?php echo wp_kses_post( $chk_label ); ?>
									</span>
									&nbsp;&nbsp;
								<?php
							}
