...
class Wt_Import_Export_For_Woo_Review_Request {
	 ...
	 *
	 * @since    1.0.9
	 * @var      int    $remind_days2    Remind interval in days, when did the banner to show
	 */
	private $second_remind_days = 60; /* remind interval in days */
	...
	public function __construct() {
		...
		if ( $this->check_condition() ) {
			...
			$this->banner_message = sprintf( __( 'Thank you for using %1$sWebToffee%2$s\'s Import Export Suite for WooCommerce. We hope you are enjoying the plugin. If you have a moment, please leave us a review. Your feedback helps us grow and improve!', 'import-export-suite-for-woocommerce' ), '<b>', '</b>' );
			...
			$this->review_btn_text = __( 'Leave a review', 'import-export-suite-for-woocommerce' );
	...
	public function show_banner() {
		...
		<div class="<?php echo esc_attr( $this->banner_css_class ); ?> notice-info notice is-dismissible">
			...
			if ( '' !== $this->webtoffee_logo_url ) {
				?>
					<h3 style="margin: 10px 0;"><strong><?php echo ( 'Enjoyed using Import Export Suite For WooCommerce? Share Your Experience!' ); ?></strong></h3>
				<?php
			}
			?>
			<p>
	...
	private function check_condition() {
		...
		if ( 2 === $this->current_banner_state || 5 === $this->current_banner_state ) {
			...
			$days = ( 2 === $this->current_banner_state ) ? $this->days_to_show_banner : ( 5 === $this->current_banner_state ? $this->remind_days : $this->second_remind_days );
